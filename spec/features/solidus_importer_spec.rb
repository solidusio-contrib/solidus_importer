...
RSpec.describe 'Import a product' do # rubocop:disable RSpec/DescribeClass
  subject(:described_method) { SolidusImporter::ProcessImport.new(import).process }
  ...
  context 'with a products file' do
    ...
    it 'imports some products' do
      expect { described_method }.to(
        ...
        .and(change(Spree::Variant, :count).by(4))
      )
      expect(Spree::LogEntry).to have_received(:create!).exactly(rows_count).times
      import.destroy
    end
